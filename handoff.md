...
`origin/main` at commit `833f6a4`. Lesson storage now persists in a real
...
- Доступ учеников к урокам (просмотр без редактирования) — обсуждали,
  план зафиксирован, реализация НЕ начата, вернёмся после текущей
  работы:
  1. Публичный незащищённый роут `GET /api/lesson/:id/view` (НЕ
     проходит через `requireAuth`) — отдаёт урок только для чтения.
     Нужен, потому что уже существующая функция "Kopiera länk" иначе
     сломается для учеников после того, как все `/api/lesson/*` роуты
     защитили входом школы.
  2. Флаг "режим просмотра" на существующем экране редактора (не
     отдельный экран) — прячет ВСЕ учительские контролы (Rita om, Ge
     instruktion, Arkivera, reorder, редактирование сцен и т.д.),
     оставляет только текст/иллюстрации/озвучку.
  3. Отдельная кнопка "Dela med elever" рядом с существующей "Kopiera
     länk" — копирует публичную ссылку на просмотр.
  4. НЕ делать: именные ученические аккаунты / отслеживание прогресса
     по конкретным ученикам — сознательно отклонено, ученики
     несовершеннолетние (7-9 класс), любая функция, идентифицирующая
     конкретного ребёнка, поднимает вопросы защиты данных, требующие
     отдельного рассмотрения, не чисто техническое решение.

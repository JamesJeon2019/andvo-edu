...
`origin/main` at commit `4f32861`. Lesson storage now persists in a real
...
- Прокинуты найденные диаграммы через `planLessonFromMaterial` →
  ...
  (шаг 4).
- Заземление AI-иллюстратора на реальные диаграммы учебника — полный
  пайплайн реализован и проверен end-to-end (шаги 4-6 плана переработки
  material-режима, завершают шаги 1-3 выше):
  - `generateSVG` (`illustrator.js`) принимает опциональный `grounding`
    (`{ croppedImageDataUrl, explanation }`) — при наличии передаёт
    реальную вырезанную диаграмму как визуальный референс в Vision-вызов
    (тем же паттерном, что уже использовался в `critiqueSVG`), без
    регресса для обычной (незаземлённой) генерации.
  - `grounding` прокинут через всю цепочку (`generateSVGWithRetry`,
    `generateSVGWithCritique`), включая путь регенерации после
    срабатывания критики — заземление сохраняется даже при
    перегенерации.
  - `illustrateLesson` принимает опциональный `groundingContext`
    (`{ diagrams, sourceImages }`) — для сцен с валидным `diagramIndex`
    вычисляет `grounding` через `cropDiagram`, для остальных — `null`,
    поведение без изменений.
  - `routes/lesson.js`: старый постфактум-механизм `assignSourceImages`
    ("Bildmatchning") полностью убран из пайплайна (файл
    `sceneImageMatcher.js` оставлен на диске неиспользуемым, не удалён),
    заменён новым заземлением; topic-mode (обычная генерация без
    material) не затронут.
  - Вырезанное фото диаграммы + explanation теперь персистятся на сцену
    (`scene.diagram_photo`, `scene.diagram_explanation`) и отображаются
    на фронтенде рядом с AI SVG-иллюстрацией (реальное фото как
    источник, AI-версия как упрощающее дополнение), с адаптивным
    переходом в колонку на узких экранах (переиспользован существующий
    breakpoint `@media (max-width:680px)`). Ручное переопределение
    учителем (Ersätt bild/Rita om) работает без изменений — полностью
    переопределяет весь блок иллюстрации, как и раньше.
  Подтверждено на реальном сгенерированном уроке (та же синтетическая
  страница с таблицей элементов): SVG-иллюстрация корректно отражает
  конкретные данные из настоящей диаграммы (например, точные атомные
  номера элементов первой группы — H=1, Li=3, Na=11, K=19 — взятые из
  вырезанного фото, а не додуманные моделью). ВЕСЬ план переработки
  material-режима (шаги 1-6: `detectDiagrams`, `cropDiagram`,
...
- Начата реализация многопользовательской архитектуры (school-level login,
  ...
  - Шаг 2b (`4b6733a`): `school_id` добавлен в таблицу `lessons`
    (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, nullable — старые
    тестовые уроки в БД остаются без владельца, это ожидаемо).
    `lessonStore.js` переписан: `getLesson`/`archiveLesson`/
    `deleteLesson` фильтруют `WHERE id = $1 AND school_id = $2` (не
    найдено и чужой урок дают один и тот же ответ, ID не подбирается);
    `saveLesson` устанавливает `school_id` только при INSERT, `ON
    CONFLICT DO UPDATE` его не трогает — сменить владельца урока через
    обычный save нельзя; `listLessons` требует `schoolId` и всегда
    фильтрует по нему. `requireAuth` подключён ко всем роутам
    `routes/lesson.js` через `router.use(requireAuth)` в начале файла
    (защищает и `/extract-material`), `req.session.schoolId` прокинут
    во все вызовы store-функций, включая фоновые
    `runGeneration`/`runGenerationFromMaterial` (schoolId захватывается
    из сессии ДО ухода в фон, передаётся явным параметром). Известный
    осознанный пробел: `GET /:id/status` читает из отдельного
    in-memory `progress` Map, не из `lessonStore`, поэтому не привязан
    к `school_id` — `requireAuth` требует вход, но одна вошедшая школа
    технически может опросить прогресс генерации чужой школы, зная её
    `lessonId` (утекает только прогресс на короткое окно генерации, не
    содержимое урока); оставлено как есть намеренно. Проверено на
    реальной Neon БД: две тестовые школы, урок школы A недоступен школе
    B (`GET /:id` → 404, без сессии → 401), `listLessons` школы A не
    показывает уроки школы B, `archive`/block-edit школой B на чужом ID
    — no-op/404, тем же действием школой A (владельцем) — проходит
    успешно.
  - Шаг 2c (`833f6a4`): экран входа на фронтенде (`public/index.html`,
    единственный изменённый файл). Новый экран `#login` (форма
    username/password, `#loginErrBox` — новый уникальный id, не
    переиспользует существующий `errBox`) добавлен в `SCREEN_IDS` и
    стал экраном по умолчанию по CSS (`#login{display:flex}`,
    `#setup{display:none}` — раньше было наоборот), чтобы `setup` не
    успевал мелькнуть до проверки сессии. Новая `initApp()` заменила
    прямой вызов `initFromUrl()` на `DOMContentLoaded`: сначала `GET
    /api/auth/me`, при 401 — `showScreen('login')` и остановка, при
    успехе — `currentSchool` сохраняется и запускается прежняя
    `initFromUrl()` (получила явную ветку `else { showScreen('setup')
    }`, т.к. `setup` больше не видим по умолчанию). Новые
    `submitLogin()` (привязана к submit формы, показывает "Fel
    användarnamn eller lösenord" в `loginErrBox` при 401) и `logout()`
    (`POST /api/auth/logout` → `showScreen('login')`, очищает поля
    формы). На экране `setup` рядом с "📚 Mina lektioner" добавлены имя
    школы (`currentSchool.name`) и кнопка "🚪 Logga ut". Проверено
    против реального dev-сервера через curl на уровне HTTP/сессии
    (браузерное расширение недоступно в этой сессии, пользователь
    отказался его подключать) — это покрывает всю логику, которую
    использует фронтенд: без сессии `/api/auth/me` → 401 (соответствует
    ветке `showScreen('login')`), неверный пароль → 401 с тем же
    текстом ошибки, что выводится в `loginErrBox`, верный пароль → 200
    + cookie `connect.sid` на 30 дней, повторный `/api/auth/me` с тем
    же cookie (симуляция обновления страницы) → 200 без повторного
    входа, `logout` → сессия уничтожена, следующий `/api/auth/me` →
    401.
  Шаг 2c завершён — экран входа на фронтенде (login/logout, auth gate
  на initApp, отображение имени школы), закрывает весь план
  многопользовательской архитектуры (школы): таблица schools,
  хеширование паролей, сессии в Postgres, защита всех роутов уроков
  через requireAuth, фронтенд-вход. Полностью многопользовательская
  архитектура (этапы 1, 2a, 2b, 2c) — ЗАВЕРШЕНА.
- Доступ учеников к урокам (просмотр без редактирования) — реализован
  полностью (`95199c4`, `4f32861`):
  - `GET /api/lesson/:id/view` — публичный роут (размещён до
    `router.use(requireAuth)` в `routes/lesson.js`, с явным
    комментарием почему), отдаёт урок без требования входа.
    `getLessonPublic(id)` в `lessonStore.js` — без фильтра по
    `school_id`, намеренно.
  - Фронтенд: `?view=<id>` в URL полностью обходит проверку
    авторизации (`initApp` проверяет этот параметр раньше вызова
    `/api/auth/me`). `viewMode` скрывает все учительские контролы
    (Kopiera länk, Arkivera, block-actions, AI-rewrite, sceneControls)
    — остаётся только контент, озвучка, иллюстрации. Скрытые учителем
    блоки (`visible: false`) полностью отфильтрованы из сайдбара И из
    пагинации (найден и исправлен реальный баг при трассировке логики
    — "Nästa →" мог провести ученика на скрытый блок в обход фильтра
    сайдбара, поскольку пагинация использовала `idx±1` напрямую вместо
    поиска ближайшего видимого блока). Существующая кнопка-заглушка
    "👁 Elevvy →" подключена к реальной функции `copyStudentLink()`.
  Проверено сквозным сценарием: публичная ссылка открывается без
  входа, учительские кнопки отсутствуют, скрытые блоки недостижимы ни
...
- Решение уже принято (многопользовательская архитектура теперь
  ЗАВЕРШЕНА, см. "What was completed" выше): НЕ использовать Google
  OAuth/Classroom для авторизации сейчас — Google Classroom в Швеции
  завязан на Skolverket, это отдельный, гораздо более глобальный
  уровень интеграции, не относящийся к текущей цели (продажа отдельным
  школам как самостоятельный инструмент между учителями внутри школы,
  не национальная инфраструктура). Google Classroom интеграция отложена
  как отдельная, возможно нереализуемая для отдельных школ тема — не
  блокирует и не влияет на текущую архитектуру.
- Дополнительная будущая идея (обсуждали, для другой сессии, если
  решим делать интерактивный тест): блок `test` в текущем коде
  (`writer.js`, `writeBlock`) НЕ структурирован — это просто текст
  вопроса в `voice_text`, БЕЗ поля правильного ответа в данных вообще.
  Прежде чем строить любую интерактивность (самопроверка кликом, или с
  отслеживанием "кто ответил" по паттерну как у других похожих
  EdTech-инструментов — например, Edcafe AI, где ученик просто вписывает
  имя без создания аккаунта) — нужно сначала переделать формат этого
  блока на структурированный (`{ question, options[], correctAnswer,
  explanation }`). Это единственная действительно "дорогая для
  retrofit" часть — сама видимость результатов для учителя (отдельная
  таблица `submissions` + экран результатов) технически лёгкая
  надстройка поверх уже структурированных данных, можно добавить в
  любой момент без риска.
